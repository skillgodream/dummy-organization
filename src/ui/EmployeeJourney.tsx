...
export function EmployeeJourney({ employee, onBack }: { employee: any, onBack: () => void }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/internal/lab/record', {
      method: 'POST',
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12">
      ...
      <div className="w-full lg:w-72 shrink-0 flex flex-col gap-8 lg:gap-10">
        <div>
          <button onClick={onBack} className="text-sm font-medium text-slate-500 hover:text-slate-900 mb-6 flex items-center gap-2 transition-colors py-2 -ml-2 px-2 rounded-lg">
            ← Back to Lab
          </button>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900 mb-1">{employee.name}</h1>
          <p className="text-sm font-mono font-medium text-slate-500">{employee.id}</p>
        </div>

        <div>
          <h3 className="text-xs font-semibold text-slate-400 mb-4 uppercase tracking-widest">10-Day Journey</h3>
            ...
            {Array.from({ length: 11 }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative w-12 h-12 flex items-center justify-center rounded-lg border text-sm font-medium transition-all ${
                    selectedDay === i 
          ...
          <button 
            ...
            className="mt-6 w-full py-2.5 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
          >
            Restart Journey
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm">
          ...
          <div className="mt-8 border-t border-slate-200 bg-slate-50 -mx-5 sm:-mx-6 md:-mx-8 lg:-mx-10 -mb-5 sm:-mb-6 md:-mb-8 lg:-mb-10 p-5 sm:p-6 md:p-8 lg:p-10 rounded-b-2xl flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
            <button 
              ...
              className="w-full sm:w-auto py-3 sm:py-2 px-4 -ml-4 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 sm:hover:bg-transparent rounded-lg font-medium transition-colors"
            >
              Clear Day {selectedDay}
            </button>
            <button 
              type="submit"
              className="w-full sm:w-auto px-6 py-3 sm:py-2.5 bg-slate-900 text-white rounded-lg font-medium hover:bg-slate-800 transition-colors shadow-sm"
            >
...
function ExpandableSection({ title, isActive, onToggle, children }: { title: string, isActive: boolean, onToggle: () => void, children: React.ReactNode }) {
  return (
    <div className={`border rounded-xl transition-all overflow-hidden ${isActive ? 'border-slate-300 shadow-sm bg-white' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
      <button
        type="button"
        onClick={onToggle}
        className="w-full px-6 py-4 bg-slate-50 flex justify-between items-center text-left focus:outline-none hover:bg-slate-100/50 transition-colors"
      >
        <h3 className={`text-sm font-semibold uppercase tracking-widest ${isActive ? 'text-slate-900' : 'text-slate-500'}`}>
          {title}
        </h3>
        <svg 
          className={`w-5 h-5 text-slate-400 transform transition-transform duration-200 ${isActive ? 'rotate-180 text-slate-600' : ''}`} 
          fill="none" 
          viewBox="0 0 24 24" 
          stroke="currentColor"
        >
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <select 
        value={value || ''} 
        onChange={e => onChange(e.target.value)} 
        className="px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"
      >
        {options.map(o => (
  ...
  return (
    <div className="flex flex-col gap-1.5 col-span-1 md:col-span-2">
      ...
      <textarea 
        ...
        onChange={e => onChange(e.target.value)} 
        rows={2}
        className="px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all placeholder:text-slate-400"
      />
    </div>
  );
}
